...
ax_violin = sns.violinplot(data = df_t1_c1, x = 'size_string', y = 'max', hue = 'mode', hue_order = ['datatype', 'manual'], ax = axes, facet_kws={'legend_out': True}, palette={'datatype':'tab:blue', 'manual':'tab:orange'}, inner=None)
a = sns.pointplot(data = df_t1_c1, x = 'size_string', y = 'max', hue = 'mode', hue_order = ['datatype', 'manual'], dodge = 0.4, ax = ax_violin, estimator=np.median, color='black', join=False, scale=0.5, errwidth=1, capsize=0.02)
...
ax_violin = sns.violinplot(data = df_c1_lsize, x = 'threads', y = 'max', order=sorted(df_c1_lsize['threads'].unique()),ax = axes, palette=['tab:blue', 'tab:orange', 'tab:orange', 'tab:orange', 'tab:orange'], inner=None)
a = sns.pointplot(data = df_c1_lsize, x = 'threads', y = 'max', order=sorted(df_c1_lsize['threads'].unique()), ax = ax_violin, estimator=np.median, color='black', join=False, scale=0.5, errwidth=1, capsize=0.02)
...
plt.legend(handles=[elems[0], elems[4]],labels=["Custom datatype", "Manual"], loc='upper left')
...
sns.set_style('darkgrid')
fig, axes = plt.subplots()
ax_violin = sns.violinplot(data = df_t1_lsize, x = 'chunk', y = 'max', ax = axes, palette=['tab:blue', 'tab:orange', 'tab:orange', 'tab:orange', 'tab:orange', 'tab:orange'], inner=None)
a = sns.pointplot(data = df_t1_lsize, x = 'chunk', y = 'max', order=sorted(df_t1_lsize['chunk'].unique()), ax = ax_violin, estimator=np.median, color='black', join=False, scale=0.5, errwidth=1, capsize=0.02)
...
sns.set_style('darkgrid')
fig, axes = plt.subplots()
ax_violin = sns.violinplot(data = df_t1_lsize, x = 'chunk', y = 'max', ax = axes, order=sorted(df_t1_lsize['chunk'].unique()), palette=['tab:blue', 'tab:orange', 'tab:orange', 'tab:orange', 'tab:orange', 'tab:orange'], inner=None)
a = sns.pointplot(data = df_t1_lsize, x = 'chunk', y = 'max', order=sorted(df_t1_lsize['chunk'].unique()), ax = ax_violin, estimator=np.median, color='black', join=False, scale=0.5, errwidth=1, capsize=0.02)
plt.xlabel('Chunk number')
plt.ylabel("lg time(us)")
axes.set_xticklabels(['datatype', '1', '2', '3', '4', '5'])
...
sns.set_style('darkgrid')
fig, axes = plt.subplots()
ax_violin = sns.violinplot(data = df_c1_lsize, x = 'size_string', y = 'max', hue = 'mode', hue_order = ['datatype', 'manual', 'put_manual'], ax = axes, facet_kws={'legend_out': True}, palette={'datatype':'tab:blue', 'manual':'tab:orange', 'put_manual':'tab:green'}, inner=None)
a = sns.pointplot(data = df_c1_lsize, x = 'size_string', y = 'max', hue = 'mode', hue_order = ['datatype', 'manual', 'put_manual'], ax = ax_violin, dodge = 0.53, estimator=np.median, color='black', join=False, scale=0.5, errwidth=1, capsize=0.02)
plt.tick_params(axis='x', which='major', labelsize=9)
plt.xlabel('Block size')
plt.ylabel("lg time(us)")
...
handles, labels = a.get_legend_handles_labels()
a.legend(handles, labels[:3], loc='upper left')
new_labels = ['Custom datatype', 'Manual', 'One-sided']
for t, l in zip(a.get_legend().texts, new_labels):
    t.set_text(l)
