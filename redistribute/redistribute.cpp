...
int main(int argc, char** argv)
{
	...
	for (int size_multiple = 20; size_multiple <= 30; ++size_multiple)
	{
		...
		std::vector<int> pgrid_send {2, 1, 1, 3, 2};
		std::vector<int> pgrid_recv {1, 1, 1, 1, 12};
		std::vector<int> total_array_size {2*size_multiple, 1*size_multiple, 1*size_multiple, 3*size_multiple, 12*size_multiple};
		...
		for (int threads = 1; threads <= 8; ++threads)
		{
			...
			for (int num_chunks = 1; num_chunks <= 8; ++num_chunks)
			{
				...
				for (int i=0; modes[i] != NULL; i++)
				{
					...
					// LSB timing setup
					std::string name = "5d_redistribute_" + MODE + "_t" + std::to_string(omp_get_max_threads()) + "_c" + std::to_string(num_chunks) + "_s" + std::to_string(state->send_element_count * 4);
					...
					LSB_Set_Rparam_int("size", state->send_element_count * 4);
