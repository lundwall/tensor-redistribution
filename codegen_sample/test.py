...
@dace.program
def matrix_1d_1d(A: dace.int32[m, m*P]):
	a_grid = dace.comm.Cart_create([P, 1])
	b_grid = dace.comm.Cart_create([1, P])
	B = np.empty_like(A, shape=(m*P, m))
	a_arr = dace.comm.Subarray((m*P, m*P), A, process_grid=a_grid)
	b_arr = dace.comm.Subarray((m*P, m*P), B, process_grid=b_grid)
